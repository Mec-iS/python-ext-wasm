//! The `Instance` Python object to build WebAssembly instances, used via `wasmer_runtime.Instance`
//!
//! *Reminder*:
//! > `WebAssembly.Instance` object is a stateful, executable instance of a
//! > `WebAssembly.Module`. Instance objects contain all the Exported WebAssembly functions
//! > that allow calling into WebAssembly code
...
/// It is implemented using PyO3 Python class that defines __call__, see related `impl`.
...
#[pyclass]
/// A struct that binds a collection of exported functions (a namespace) to `wasmer_runtime.Memory`
